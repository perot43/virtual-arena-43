...
app.get('/api/availability',(req,res)=>{const date=String(req.query.date||'');if(!validDate(date))return res.status(400).json({error:'Data non valida'});const closedDay=!!db.prepare(`SELECT 1 FROM blocked_slots WHERE block_date=? AND block_time IS NULL`).get(date);if(closedDay)return res.json({date,slots:[],escapeSunday:isEscapeSunday(date)});const escape=isEscapeSunday(date);res.json({date,escapeSunday:escape,slots:slotsForDate(date).map(time=>({time,available:isBlocked(date,time)?0:(escape?(activeBookingsCount(date,time)>0?0:4):Math.max(0,4-occupied(date,time))),blocked:isBlocked(date,time)}))})});
