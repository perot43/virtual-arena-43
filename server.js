...
app.get('/api/admin/config',(req,res)=>{if(!adminOK(req))return res.status(401).json({error:'Password admin errata'});res.json({schedule:schedule(),prices:prices(),escapeSunday:ESCAPE_SUNDAY,specialOpenings:specialOpenings(),blocked:db.prepare(`SELECT * FROM blocked_slots ORDER BY block_date,block_time`).all(),discounts:db.prepare(`SELECT code,percent,active FROM discounts ORDER BY code`).all()})});
