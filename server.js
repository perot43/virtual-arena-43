...
function slotsForDate(d){if(!validDate(d))return[];const dow=new Date(`${d}T12:00:00`).getDay();return schedule()[dow]||[]}
...
function baseTotal(p){const pr=prices();return Number(pr[p]??DEFAULT_PRICES[p])}
function discountedTotal(p,raw=''){const d=discountInfo(raw),base=baseTotal(p);return{code:d?.code||'',percent:d?.percent||0,total:d?Math.round(base*(100-d.percent)/100):base}}
...
app.get('/api/config',(req,res)=>{res.json({schedule:schedule(),prices:prices()})});
app.get('/api/availability',(req,res)=>{const date=String(req.query.date||'');if(!validDate(date))return res.status(400).json({error:'Data non valida'});const closedDay=!!db.prepare(`SELECT 1 FROM blocked_slots WHERE block_date=? AND block_time IS NULL`).get(date);if(closedDay)return res.json({date,slots:[]});res.json({date,slots:slotsForDate(date).map(time=>({time,available:isBlocked(date,time)?0:Math.max(0,4-occupied(date,time)),blocked:isBlocked(date,time)}))})});
...
app.patch('/api/admin/bookings/:id/players',(req,res)=>{if(!adminOK(req))return res.status(401).json({error:'Password admin errata'});const id=Number(req.params.id),p=Number(req.body?.players);if(!Number.isInteger(p)||p<1||p>4)return res.status(400).json({error:'Numero giocatori non valido'});const b=db.prepare(`SELECT * FROM bookings WHERE id=?`).get(id);if(!b)return res.status(404).json({error:'Prenotazione non trovata'});if(!['confirmed','pending_payment'].includes(b.status))return res.status(400).json({error:'Prenotazione non attiva'});if(occupied(b.booking_date,b.booking_time,id)+p>4)return res.status(409).json({error:'Non ci sono abbastanza posti nello slot'});const total=b.payment_status==='paid'?b.total_cents:discountedTotal(p,b.discount_code).total;db.prepare(`UPDATE bookings SET players=?,total_cents=? WHERE id=?`).run(p,total,id);res.json({ok:true})});
app.patch('/api/admin/bookings/:id/move',(req,res)=>{if(!adminOK(req))return res.status(401).json({error:'Password admin errata'});const id=Number(req.params.id),date=String(req.body?.date||''),time=String(req.body?.time||'');const b=db.prepare(`SELECT * FROM bookings WHERE id=?`).get(id);if(!b)return res.status(404).json({error:'Prenotazione non trovata'});if(!validSlot(date,time))return res.status(400).json({error:'Nuovo slot non valido o bloccato'});if(occupied(date,time,id)+b.players>4)return res.status(409).json({error:'Non ci sono abbastanza posti nel nuovo slot'});db.prepare(`UPDATE bookings SET booking_date=?,booking_time=? WHERE id=?`).run(date,time,id);res.json({ok:true})});
...
app.post('/api/admin/bookings/manual',(req,res)=>{if(!adminOK(req))return res.status(401).json({error:'Password admin errata'});const{date,time,players,name,phone='',email='',notes='',paid=false}=req.body||{},p=Number(players);if(!validSlot(date,time)||![1,2,3,4].includes(p)||!name)return res.status(400).json({error:'Dati prenotazione non validi'});const c=code(),calc=discountedTotal(p,'');try{insertTx({booking_code:c,booking_date:date,booking_time:time,players:p,total_cents:calc.total,name:String(name).trim(),phone:String(phone).trim()||'-',email:String(email).trim()||'manuale@virtualarena43.it',notes:String(notes).trim(),payment_method:'arena',payment_status:paid?'paid':'pay_at_venue',status:'confirmed',discount_code:'',billing_first_name:'',billing_last_name:'',billing_address:'',billing_tax_code:'',internal_notes:'Inserita manualmente'});res.json({ok:true,bookingCode:c})}catch(e){if(e.message==='SLOT_FULL')return res.status(409).json({error:'Slot pieno'});throw e}});
app.post('/api/admin/blocks',(req,res)=>{if(!adminOK(req))return res.status(401).json({error:'Password admin errata'});const date=String(req.body?.date||''),time=req.body?.time?String(req.body.time):null,reason=String(req.body?.reason||'');if(!validDate(date))return res.status(400).json({error:'Data non valida'});if(time)db.prepare(`DELETE FROM blocked_slots WHERE block_date=? AND block_time=?`).run(date,time);else db.prepare(`DELETE FROM blocked_slots WHERE block_date=? AND block_time IS NULL`).run(date);db.prepare(`INSERT INTO blocked_slots(block_date,block_time,reason) VALUES(?,?,?)`).run(date,time,reason);res.json({ok:true})});
