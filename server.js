...
CREATE TABLE IF NOT EXISTS special_openings(
 id INTEGER PRIMARY KEY AUTOINCREMENT,
 opening_date TEXT NOT NULL,
 opening_time TEXT NOT NULL,
 UNIQUE(opening_date,opening_time)
);
...
function discountsEnabled(){return getSetting('discounts_enabled',true)!==false}
...
function specialSlotsForDate(d){return db.prepare(`SELECT opening_time FROM special_openings WHERE opening_date=? ORDER BY opening_time`).all(d).map(x=>x.opening_time)}
function slotsForDate(d){if(!validDate(d))return[];const base=isEscapeSunday(d)?ESCAPE_SUNDAY.slots:(schedule()[new Date(`${d}T12:00:00`).getDay()]||[]);return[...new Set([...base,...specialSlotsForDate(d)])].sort()}
...
app.post('/api/discount',(req,res)=>{if(!discountsEnabled())return res.status(404).json({valid:false,error:'Codici sconto non disponibili'});const d=discountInfo(req.body?.code);if(!d)return res.status(404).json({valid:false,error:'Codice sconto non valido'});res.json({valid:true,code:d.code,percent:d.percent})});
...
app.get('/api/admin/config',(req,res)=>{if(!adminOK(req))return res.status(401).json({error:'Password admin errata'});res.json({schedule:schedule(),prices:prices(),escapeSunday:ESCAPE_SUNDAY,specialOpenings:specialOpenings(),discountsEnabled:discountsEnabled(),blocked:db.prepare(`SELECT * FROM blocked_slots ORDER BY block_date,block_time`).all(),discounts:db.prepare(`SELECT code,percent,active FROM discounts ORDER BY code`).all()})});
app.put('/api/admin/config',(req,res)=>{if(!adminOK(req))return res.status(401).json({error:'Password admin errata'});const s=req.body?.schedule,p=req.body?.prices;if(s){const clean={};for(const [k,v] of Object.entries(s)){const day=Number(k);if(day<0||day>6||!Array.isArray(v))continue;clean[day]=[...new Set(v.map(x=>String(x).trim()).filter(x=>/^\d{2}:\d{2}$/.test(x)))].sort()}setSetting('schedule',clean)}if(p){const clean={};for(let n=1;n<=4;n++){const cents=Math.round(Number(p[n]));if(!Number.isFinite(cents)||cents<0)return res.status(400).json({error:'Prezzi non validi'});clean[n]=cents}setSetting('prices',clean)}if(typeof req.body?.discountsEnabled==='boolean')setSetting('discounts_enabled',req.body.discountsEnabled);res.json({ok:true,schedule:schedule(),prices:prices(),escapeSunday:ESCAPE_SUNDAY,discountsEnabled:discountsEnabled()})});
...
app.post('/api/admin/bookings/manual',async(req,res)=>{if(!adminOK(req))return res.status(401).json({error:'Password admin errata'});const{date,time,players,name,phone='',email='',notes='',paid=false,sendEmail=false}=req.body||{},p=Number(players),escape=isEscapeSunday(date),customerEmail=String(email).trim();if(!validSlot(date,time)||(escape?![2,3,4].includes(p):![1,2,3,4].includes(p))||!name)return res.status(400).json({error:'Dati prenotazione non validi'});if(sendEmail&&!customerEmail)return res.status(400).json({error:'Inserisci l’email del cliente per inviare la conferma'});const c=code(),calc=discountedTotal(p,'',date);try{insertTx({booking_code:c,booking_date:date,booking_time:time,players:p,total_cents:calc.total,name:String(name).trim(),phone:String(phone).trim()||'-',email:customerEmail||'manuale@virtualarena43.it',notes:escape?`[ESCAPE SUNDAY] ${String(notes).trim()}`:String(notes).trim(),payment_method:'arena',payment_status:paid?'paid':'pay_at_venue',status:'confirmed',discount_code:'',billing_first_name:'',billing_last_name:'',billing_address:'',billing_tax_code:'',internal_notes:'Inserita manualmente'});if(sendEmail)await sendBookingEmails(c);res.json({ok:true,bookingCode:c,emailRequested:!!sendEmail,emailSent:!!bookingByCode(c)?.email_sent_at})}catch(e){if(e.message==='SLOT_FULL')return res.status(409).json({error:'Slot pieno'});throw e}});
app.post('/api/admin/special-openings',(req,res)=>{if(!adminOK(req))return res.status(401).json({error:'Password admin errata'});const date=String(req.body?.date||''),times=Array.isArray(req.body?.times)?req.body.times.map(x=>String(x).trim()):[],opening=new Date('2026-10-31T12:00:00');if(!validDate(date)||new Date(`${date}T12:00:00`)<opening)return res.status(400).json({error:'La giornata extra deve essere dal 31 ottobre 2026 in poi'});const clean=[...new Set(times.filter(x=>/^([01]\d|2[0-3]):[0-5]\d$/.test(x)))].sort();if(!clean.length)return res.status(400).json({error:'Seleziona almeno un orario'});const add=db.prepare(`INSERT OR IGNORE INTO special_openings(opening_date,opening_time) VALUES(?,?)`);db.transaction(()=>clean.forEach(t=>add.run(date,t)))();res.json({ok:true,date,times:slotsForDate(date)})});
app.delete('/api/admin/special-openings',(req,res)=>{if(!adminOK(req))return res.status(401).json({error:'Password admin errata'});const date=String(req.body?.date||''),time=String(req.body?.time||'');if(!validDate(date)||!/^([01]\d|2[0-3]):[0-5]\d$/.test(time))return res.status(400).json({error:'Data o orario non validi'});db.prepare(`DELETE FROM special_openings WHERE opening_date=? AND opening_time=?`).run(date,time);res.json({ok:true})});
